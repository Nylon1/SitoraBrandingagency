...
export function RevenueIntelligenceView() {
  return <div>
    ...
    <div className="mt-5 grid gap-5 2xl:grid-cols-[1.4fr_0.8fr]">
      <Panel title="Opportunity queue" subtitle="Prioritised by value, age and likelihood of recovery" action={<span className="rounded-lg border border-white/[0.07] px-3 py-1.5 text-[9px] text-white/30">Highest-value examples</span>}>
...
export function ClaimsView() {
  return <div>
    ...
    <div className="mt-5 grid gap-5 2xl:grid-cols-[1.4fr_0.8fr]">
      ...
      <Panel title="Pattern detected" subtitle="Repeated exception clusters"><div className="rounded-2xl border border-amber-400/18 bg-amber-400/[0.045] p-4"><div className="flex items-center gap-2 text-[10px] font-semibold uppercase tracking-[0.13em] text-amber-200"><AlertTriangle size={13}/>JED-SUP-01</div><div className="mt-3 text-[15px] font-medium">11 Jeddah claims share the same supporting-information pattern.</div><p className="mt-2 text-[11px] leading-5 text-white/40">Handle this as one workflow issue rather than 11 unrelated claim failures. Estimated exposed value: SAR 41,600.</p><Link href="/tools/dental-control/actions-live" className="mt-4 inline-flex items-center gap-2 text-[10px] font-medium text-amber-100">Create grouped action <ArrowRight size={11}/></Link></div><div className="mt-3 rounded-2xl border border-white/[0.055] bg-white/[0.025] p-4"><div className="text-[10px] text-white/28">Prototype guardrail</div><p className="mt-2 text-[10px] leading-5 text-white/42">Rules validate coded workflows. AI can explain patterns and prioritise actions, but it does not invent dental codes or submit claims in this prototype.</p></div></Panel>
    </div>
  </div>;
}
...
export function ActionCentreView() {
  return <div>
    <PageIntro eyebrow="Action centre" title="Turn intelligence into accountable work." copy="Every useful insight should end in an owner, a due date and a resolution state. This is what moves Sitora beyond business intelligence into an operating layer." right={<Link href="/tools/dental-control/actions-live" className="rounded-xl bg-[#2aa89a] px-4 py-2.5 text-[11px] font-semibold text-[#04110f]">Open live actions</Link>} />
