...
function AnswerCard({ answer, onFollowUp }: { answer: CopilotAnswer; onFollowUp: (q: string) => void }) {
  return <div className="mt-3 rounded-3xl border border-[#2aa89a]/16 bg-gradient-to-br from-[#0d2822] to-[#0a1916] p-4 md:p-5">
    ...
    <div className="mt-4 grid gap-3 lg:grid-cols-[1.15fr_0.85fr]">
      <div className="rounded-2xl border border-white/[0.06] bg-white/[0.025] p-4"><div className="flex items-center gap-2 text-[10px] font-semibold text-white/60"><Link2 size={12} className="text-[#76cbbf]"/> Evidence trail</div><div className="mt-3 space-y-2">{answer.evidence.map((item) => {
        const inner = <div className="flex items-center justify-between gap-3 rounded-xl border border-white/[0.05] bg-white/[0.02] p-3 transition hover:bg-white/[0.035]"><div><div className="text-[10px] font-medium text-white/68">{item.label}</div><div className="mt-1"><span className={`rounded-full border px-2 py-0.5 text-[8px] uppercase tracking-[0.1em] ${typeTone[item.type]}`}>{item.type}</span></div></div><div className="flex items-center gap-2 text-[10px] text-white/45">{item.value}{item.href ? <ChevronRight size={11}/> : null}</div></div>;
...
export function ExecutiveCopilot() {
  ...
  function ask(question: string) {
    ...
    if (!q) return;
    const now = Date.now();
    setMessages((current) => [...current, { id: now, role: "user", text: q }, { id: now + 1, role: "assistant", text: "", answer: getExecutiveAnswer(q) }]);
    setInput("");
  }
  ...
  return <div className="min-h-screen bg-[#071310] text-white">
    ...
    <main className="mx-auto grid max-w-[1500px] gap-5 px-4 py-5 md:px-7 lg:grid-cols-[1fr_340px]">
      <section className="min-w-0 rounded-3xl border border-white/[0.07] bg-white/[0.02]">
        <div className="border-b border-white/[0.06] p-5"><div className="flex items-center gap-2 text-[12px] font-semibold"><Bot size={15} className="text-[#75ccc1]"/> Evidence-grounded executive conversation</div><p className="mt-2 max-w-3xl text-[10px] leading-5 text-white/30">This prototype does not just generate prose. Every material answer exposes its evidence trail, confidence, assumptions and recommended next action.</p><div className="mt-4 flex flex-wrap gap-2">{executivePrompts.map((q) => <button key={q} type="button" onClick={() => ask(q)} className="rounded-full border border-white/[0.07] bg-white/[0.025] px-3 py-1.5 text-[9px] text-white/42 hover:border-[#2aa89a]/25 hover:text-white/70">{q}</button>)}</div></div>
