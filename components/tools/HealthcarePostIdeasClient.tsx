...
export function HealthcarePostIdeasClient() {
  const [category, setCategory] = useState(dentalCategories[0].slug);
  const [query, setQuery] = useState("");
  const [format, setFormat] = useState<(typeof formats)[number]>("All formats");
  const [angle, setAngle] = useState<(typeof angles)[number]>("All angles");
  const [visibleCount, setVisibleCount] = useState(12);
  const [selectedIdea, setSelectedIdea] = useState<HealthcarePostIdea | null>(null);
    ...
    return dentalPostIdeas.filter((idea) => {
      ...
      const queryMatch =
        ...
        `${idea.title} ${idea.hook} ${idea.topic} ${idea.category}`
          .toLowerCase()
          .includes(searchTerm);
      const formatMatch = format === "All formats" || idea.format === format;
      const angleMatch = angle === "All angles" || idea.angle === angle;

      return categoryMatch && queryMatch && formatMatch && angleMatch;
    });
  }, [angle, category, format, query, savedIds, showSaved]);

  const visibleIdeas = results.slice(0, visibleCount);
  const currentCategory = dentalCategories.find((item) => item.slug === category);
  ...
  function surpriseMe() {
    const idea = dentalPostIdeas[Math.floor(Math.random() * dentalPostIdeas.length)];
        ...
        <div className="mx-auto max-w-7xl">
            ...
            <div className="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
              <div className="max-w-4xl">
                ...
                <p className="mt-6 max-w-3xl text-base leading-8 text-white/55 sm:text-lg">
                  Choose your profession, browse focused categories and find useful social media ideas without creating an account.
                </p>

                <div className="mt-6 flex flex-wrap gap-2 text-xs font-semibold text-white/55">
                  <span className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2">No signup</span>
                  <span className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2">500 dentist ideas</span>
                  <span className="rounded-full border border-white/10 bg-white/[0.04] px-4 py-2">Free to use</span>
                </div>
              </div>
          ...
          <section className="mt-10" aria-labelledby="profession-heading">
            <div className="mb-5 flex flex-wrap items-end justify-between gap-3">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#d8b66d]">Step one</p>
                <h2 id="profession-heading" className="mt-2 text-3xl font-semibold tracking-tight">Choose your profession</h2>
              </div>
              <p className="text-sm text-white/38">Doctors and opticians are coming next.</p>
            </div>

            <div className="grid gap-4 sm:grid-cols-3">
              <div className="rounded-[26px] border border-[#d8b66d]/40 bg-[#d8b66d]/10 p-6">
                <div className="flex items-start justify-between gap-4">
                  <span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#d8b66d] text-[#070910]">
                    <Stethoscope className="h-6 w-6" />
                  </span>
                  <span className="rounded-full bg-emerald-400/10 px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-emerald-300">Live</span>
                </div>
                <h3 className="mt-6 text-2xl font-semibold">Dentist</h3>
                <p className="mt-2 text-sm leading-6 text-white/50">10 categories and {dentalIdeaCount} ideas.</p>
              </div>

              {["Doctor / GP", "Optician"].map((profession) => (
                <div key={profession} className="rounded-[26px] border border-white/10 bg-white/[0.035] p-6 opacity-65">
                  <div className="flex items-start justify-between gap-4">
                    <span className="grid h-12 w-12 place-items-center rounded-2xl border border-white/10 bg-white/[0.045] text-white/45">
                      <Stethoscope className="h-6 w-6" />
                    </span>
                    <span className="rounded-full bg-white/[0.06] px-3 py-1.5 text-[10px] font-bold uppercase tracking-[0.16em] text-white/35">Coming next</span>
                  </div>
                  <h3 className="mt-6 text-2xl font-semibold text-white/65">{profession}</h3>
                  <p className="mt-2 text-sm leading-6 text-white/35">The same simple, no-signup experience.</p>
                </div>
              ))}
            </div>
          </section>
          ...
          <section className="mt-12" aria-labelledby="category-heading">
            <div className="mb-5 flex flex-wrap items-end justify-between gap-4">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#d8b66d]">Step two</p>
                <h2 id="category-heading" className="mt-2 text-3xl font-semibold tracking-tight">Browse ten content categories</h2>
              </div>
              <button
                type="button"
                onClick={() => {
              ...
              >
                ...
                Saved ideas ({savedIds.length})
              </button>
            </div>

            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">
              {dentalCategories.map((item, index) => {
                ...
                return (
                  ...
                  >
                    <div className="flex items-center justify-between gap-3">
                      <span className="text-xs font-bold text-[#d8b66d]">{String(index + 1).padStart(2, "0")}</span>
                      <span className="rounded-full bg-white/[0.06] px-2.5 py-1 text-[10px] font-bold text-white/45">50</span>
                    </div>
                    <h3 className="mt-4 font-semibold leading-6">{item.name}</h3>
                    <p className="mt-2 line-clamp-3 text-xs leading-5 text-white/38">{item.description}</p>
                  </button>
                );
              })}
            </div>
          ...
          <section className="mt-12 rounded-[30px] border border-white/10 bg-white/[0.035] p-5 sm:p-8" aria-labelledby="ideas-heading">
            <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#d8b66d]">Step three</p>
                <h2 id="ideas-heading" className="mt-2 text-3xl font-semibold tracking-tight">
                  {showSaved ? "Your saved ideas" : currentCategory?.name}
                </h2>
                <p className="mt-2 text-sm text-white/42">{results.length} ideas match your selection.</p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-3 lg:w-[680px]">
                ...
                <label>
                  ...
                  <select
                    value={angle}
                    onChange={(event) => {
                      setAngle(event.target.value as (typeof angles)[number]);
                      setVisibleCount(12);
                    }}
                    className="w-full rounded-2xl border border-white/10 bg-[#090b11] px-4 py-3 text-sm text-white outline-none focus:border-[#d8b66d]/50"
                  >
            ...
            {visibleIdeas.length ? (
                    ...
                    >
                      <div className="flex items-start justify-between gap-4">
                        <div className="flex flex-wrap gap-2">
                          <span className="rounded-full bg-[#d8b66d]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[#e9ca82]">{idea.format}</span>
                          <span className="rounded-full bg-white/[0.05] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-white/42">{idea.angle}</span>
                        </div>
                        <button
                          type="button"
                      ...
                      <h3 className="mt-5 text-xl font-semibold leading-8 tracking-tight sm:text-2xl">{idea.title}</h3>
                      <p className="mt-3 text-sm leading-7 text-white/48">{idea.hook}</p>
                      ...
                      <div className="mt-6 flex flex-wrap items-center justify-between gap-3 border-t border-white/10 pt-5">
                        ...
                        >
                          {copiedId === idea.id ? <Check className="h-4 w-4 text-emerald-300" /> : <Copy className="h-4 w-4" />}
              ...
              <div className="mt-8 rounded-[26px] border border-dashed border-white/15 px-6 py-16 text-center">
                ...
                <h3 className="mt-4 text-xl font-semibold">No matching ideas</h3>
                <p className="mt-2 text-sm text-white/42">Try another search, format or content angle.</p>
              </div>
            )}
          ...
          <div className="mt-10 rounded-[30px] border border-[#d8b66d]/20 bg-[#d8b66d]/[0.07] px-6 py-9 text-center sm:px-10">
            <p className="text-xs font-bold uppercase tracking-[0.22em] text-[#d8b66d]">Healthcare content note</p>
            <p className="mx-auto mt-4 max-w-3xl text-sm leading-7 text-white/50">
              These prompts are starting points for educational marketing. Clinics remain responsible for checking accuracy, professional standards, consent and advertising claims before publishing.
            </p>
          </div>
        </div>
      </section>
      ...
      <AnimatePresence>
        {selectedIdea && (
          <motion.div
            ...
            onMouseDown={(event) => {
              if (event.currentTarget === event.target) setSelectedIdea(null);
            }}
          >
            <motion.div
            ...
            >
              <div className="flex items-start justify-between gap-5">
                <div>
                  <div className="flex flex-wrap gap-2">
                    <span className="rounded-full bg-[#d8b66d]/10 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-[#e9ca82]">{selectedIdea.format}</span>
                    <span className="rounded-full bg-white/[0.05] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.14em] text-white/42">{selectedIdea.category}</span>
                  </div>
                  <h2 id="selected-idea-title" className="mt-5 text-3xl font-semibold tracking-tight sm:text-4xl">{selectedIdea.title}</h2>
                </div>
                <button
                  type="button"
              ...
              <div className="mt-8 grid gap-4 sm:grid-cols-3">
                <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5 sm:col-span-2">
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#d8b66d]">Suggested hook</p>
                  <p className="mt-3 leading-7 text-white/65">{selectedIdea.hook}</p>
                </div>
                <div className="rounded-2xl border border-white/10 bg-white/[0.035] p-5">
                  <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#d8b66d]">Content angle</p>
                  <p className="mt-3 leading-7 text-white/65">{selectedIdea.angle}</p>
                </div>
              </div>

              <div className="mt-4 rounded-2xl border border-white/10 bg-white/[0.035] p-5">
                <p className="text-xs font-bold uppercase tracking-[0.18em] text-[#d8b66d]">Call to action</p>
                <p className="mt-3 leading-7 text-white/65">{selectedIdea.callToAction}</p>
              </div>
              ...
              <div className="mt-6 flex flex-wrap gap-3">
                ...
                >
                  {copiedId === selectedIdea.id ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}
                  {copiedId === selectedIdea.id ? "Copied" : "Copy full brief"}
                </button>
                <button
                  type="button"
                ...
                >
                  <Bookmark className="h-4 w-4" />
                  {savedIds.includes(selectedIdea.id) ? "Remove saved idea" : "Save idea"}
                </button>
              </div>
            </motion.div>
          </motion.div>
