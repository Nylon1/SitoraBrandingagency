...
export const metadata: Metadata = {
  title: "Beyond Compliance: School Health Reform 2026 | Sitora Research",
  description: "An evidence-led Sitora policy proposal on school food, health education, physical activity, transparency and continuous improvement in England.",
    ...
    <main className="min-h-screen bg-[#07110f] text-white">
      <header className="border-b border-white/[0.07] bg-[radial-gradient(circle_at_78%_0%,rgba(42,168,154,0.13),transparent_34%)]">
        <div className="mx-auto max-w-5xl px-5 py-20 md:px-8 md:py-28">
          ...
          <div className="mt-8 text-xs font-semibold uppercase tracking-[0.17em] text-[#7acdc3]">Public health & policy · August 2026</div>
          <h1 className="mt-5 text-4xl font-semibold tracking-[-0.045em] md:text-6xl">Beyond Compliance: School Health Reform 2026</h1>
          <p className="mt-6 max-w-4xl text-lg leading-8 text-white/55 md:text-xl">Reforming school food, health education and physical activity in England, with Lancashire as a 2026 case study.</p>
        </div>
      </header>
    ...
    Download full PDF
        ...
        <article className="mt-12 space-y-14">
          ...
          <section>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              {score.map(([title, points, body]) => <div key={title} className="rounded-2xl border border-[#2aa89a]/15 bg-[#0c241f] p-5"><div className="text-2xl font-semibold text-[#8bd8ce]">{points}</div><h3 className="mt-2 font-semibold">{title}</h3><p className="mt-2 text-sm leading-7 text-white/48">{body}</p></div>)}
            </div>
          </section>
          ...
          <section>
            ...
            <div className="mt-5 grid gap-3 md:grid-cols-4">
              {["1. Legislate", "2. Transition", "3. Monitor", "4. Improve first, escalate later"].map(x => <div key={x} className="rounded-xl border border-white/[0.07] bg-white/[0.02] px-4 py-4 text-sm font-medium text-white/65">{x}</div>)}
            </div>
